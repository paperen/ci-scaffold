 ...
 * @version {version}
 * @package application
 * @subpackage application/modules/user/controllers
 * @author {author}
 */
class User_Main_Module extends MY_Module
{
	...
	private function _pagination( &$data ) {
		// 总数与分页
		$total = $this->model_user_total();
		$data['total'] = $total;
		$per_page = config_item( 'per_page' );
		$this->load->library( 'pagination' );
		$pagination_config = array(
			'base_url' => module_url( 'user/main/index' ),
			'total_rows' => $total,
			'per_page' => $per_page,
			'uri_segment' => 5,
		);
	...
	public function index() {
		...
		$list_data = $this->model_user_all( config_item( 'per_page' ), $this->pagination->get_cur_offset() );
		$data['list_data'] = $list_data;

		$this->_set_pagetitle( 'user-index' );
	...
	public function view( $id ) {
		...
		try {
			...
			$item_data = $this->model_user_get_by_pk( $id );
	...
	private function _add( &$data ) {
		...
		$id = $this->model_user_insert( $post_data );
		$post_data['id'] = $id;
		$data['item_data'] = $post_data;

		init_messagebox( get_lang( 'success' ), 'success', 1, module_url( 'user/main/index' ) );
		$data['exit'] = TRUE;
	}
	...
	private function _edit( &$data ) {
		...
		$this->model_user_update( $post_data );
	...
	public function delete( $id = '' ) {
		...
		$this->_set_pagetitle( 'user-delete' );
