 ...
 * @version {version}
 * @package application
 * @subpackage application/modules/{module}/controllers
 * @author {author}
 */
class {module_upper}_Main_Module extends MY_Module
{
	...
	private function _pagination( &\$data ) {
		// 总数与分页
		\$total = \$this->model_{module}_total();
		\$data['total'] = \$total;
		\$per_page = config_item( 'per_page' );
		\$this->load->library( 'pagination' );
		\$pagination_config = array(
			'base_url' => module_url( '{module}/main/index' ),
			'total_rows' => \$total,
			'per_page' => \$per_page,
			'uri_segment' => 5,
		);
	...
	public function index() {
		...
		\$list_data = \$this->model_{module}_all( config_item( 'per_page' ), \$this->pagination->get_cur_offset() );
		\$data['list_data'] = \$list_data;

		\$this->_set_pagetitle( '{module}-index' );
	...
	public function view( \$id ) {
		...
		try {
			...
			\$item_data = \$this->model_{module}_get_by_pk( \$id );
		...
		\$this->_set_pagetitle( '{module}-view' );
	...
	public function add() {
		...
		\$this->_set_pagetitle( '{module}-add' );
	...
	private function _add( &\$data ) {
		...
		\$id = \$this->model_{module}_insert( \$post_data );
		\$post_data['id'] = \$id;
		\$data['item_data'] = \$post_data;

		init_messagebox( get_lang( 'success' ), 'success', 1, module_url( '{module}/main/index' ) );
		\$data['exit'] = TRUE;
	}
	...
	public function edit( \$id = '' ) {
		...
		\$this->_set_pagetitle( '{module}-edit' );
		\$this->load->layout( 'form', \$data );
	}
	...
	private function _edit( &\$data ) {
		...
		\$this->model_{module}_update( \$post_data );
	...
	public function delete( \$id = '' ) {
		...
		\$this->_set_pagetitle( '{module}-delete' );
